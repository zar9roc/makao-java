...
/**
 * Na początku należy zainicjować grę, a potem ją uruchomić w skład gry wchodzą
 * gracze stos kart zakrytych (nieskończony v1.0) karta odkryta
 * 
 * każdy gracz ma ruch, w którym wybiera karty do zagrania gracz może
 * zagrać jedynie karty kompatybilne z kartą już zagraną gracz może użyć więcej
 * niż jedną kartę za jednym razem po ruchu gracza następuje kolej na nastepnego
 * 
 * gdy gracz odda ostatnią kartę, wygrywa gdy zostanie jeden gracz, gra się
 * kończy
 * 
 * 
 * @author adam
...
public class game {
    ...
    public static boolean possibleToPut(int askCard, int cardBefore, boolean isItFirstCard) {
        ...
        if (gracze[turnOfPlayer].hand.get(askCard) == 11 || cardBefore == 11) {
            ...
            q = (cardBefore / 13 == gracze[turnOfPlayer].hand.get(askCard) / 13 // taki sam kolor co karta poprzednia
                    ...
                    ) || gracze[turnOfPlayer].hand.get(askCard) % 13 == cardBefore % 13); // lub taka sama karta, co
                                                                                          // ostatnia położona - dowolny
                                                                                          // kolor
    ...
    public static ArrayList<Integer> playerSelection() {
        ...
        while (!complete) {
            ...
            if (input == -1)
                ...
                inputOutput.outErrIncompatibile(cardBefore);
            // input = -2;
        }
    ...
    public static void playGame() {
        while (getPlayersInGame() >= 2) { // gracz odbębnia swoje kolejki
            ...
            if (jopekHandling > 0) {
                ...
                for (int i = player.getPlayersNum(); --i >= 0;) {
                    ArrayList<Integer> selection = getPlayerSelection(); // TODO: należy tutaj pozwolić na przebicie
                                                                         // jopkiem
            ...
            if (gracze[turnOfPlayer].tury > 0) {
                ...
                if (selection.isEmpty()) {
                    if (charge > 0) {
                        if (topCard % 13 != 3)
                            gracze[turnOfPlayer].drawCard(charge); // jeżeli jest obciążenie oraz karta na wierzchu nie
                                                                   // jest 4
                        else
                            gracze[turnOfPlayer].tury = charge;
                    } else
                        gracze[turnOfPlayer].drawCard(1);
                }
    ...
    static public void init(int numberOfPlayers, int numberOfStartingHand) {
        ...
        for (int i = numberOfPlayers; --i >= 0;) {
            gracze[i] = new player(numberOfStartingHand);
        ...
        // select first player
        turnOfPlayer = makao.random.nextInt() % player.getPlayersNum();
